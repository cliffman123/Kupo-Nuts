...
const extractMediaFromJsonResponse = async (page, link) => {
  ...
  try {
    ...
    const fileUrls = fileUrlMatches.map(match => {
      ...
      console.log('Extracted Rule34 file_url:', urlMatch ? urlMatch[1] : 'null');
    ...
    if (uniqueUrls.length > 0) {
      uniqueUrls.forEach((url, index) => {
        ...
        const convertedUrl = url.endsWith('.png') ? url.replace(/\.png$/i, '.jpeg') : url;
        mediaObjects.push({
          postLink: link,
          videoLinks: [convertedUrl],
          tags: {
            author: [],
            copyright: [],
            character: [],
...
const handleProvidedLink = async (page, providedLink, postLinksQueue, existingLinks, progressCallback, options = {}) => {
  ...
  // ✅ app.rule34 - Uses JSON extraction
  if (providedLink.includes('app.rule34')) {
    return await paginatedExtractionFlow(
      page,
      providedLink,
      existingLinks,
      progressCallback,
      extractMediaFromJsonResponse,
      getNextPageUrl,
      { ...options, siteName: 'app.rule34' }
    );
  }
  // ✅ Safebooru - Uses feed thumbnail extraction
  else if (providedLink.includes('safebooru')) {
    return await paginatedExtractionFlow(
      page,
      providedLink,
      existingLinks,
      progressCallback,
      extractMediaFromSafebooruFeed,
      getSafebooruNextPageUrl,
      { ...options, siteName: 'Safebooru' }
    );
  }
  // ✅ Other sites - Uses DOM selector flow
  else {
...
/**
 * Generate next page URL for Safebooru
 * Safebooru uses query params like ?pid=0, ?pid=42, etc.
 * Each page shows 42 posts, so pid increments by 42
 * 
 * Example:
 *   Page 1: https://safebooru.org/index.php?page=post&s=list&tags=pokemon&pid=0
 *   Page 2: https://safebooru.org/index.php?page=post&s=list&tags=pokemon&pid=42
 *   Page 3: https://safebooru.org/index.php?page=post&s=list&tags=pokemon&pid=84
 */
const getSafebooruNextPageUrl = (url, pageCount) => {
  try {
    const urlObj = new URL(url);
    const currentPid = parseInt(urlObj.searchParams.get('pid') || '0');
    const nextPid = currentPid + 42; // 42 posts per page
    
    urlObj.searchParams.set('pid', nextPid);
    return urlObj.toString();
  } catch (error) {
    console.error('Error generating Safebooru next page URL:', error);
    return null;
  }
};

/**
 * Generic Extraction Flow for paginated feed extraction
 * Works with any extraction function (app.rule34, Safebooru, etc.)
 * 
 * @param {Page} page - Playwright page object
 * @param {string} providedLink - Initial URL to scrape
 * @param {Array} existingLinks - Array to accumulate results
 * @param {Function} progressCallback - Progress reporting callback
 * @param {Function} extractionFn - Extraction function (page, url) => Promise<mediaObjects[]>
 * @param {Function} nextPageFn - Next page URL generator (url, pageCount) => string|null
 * @param {Object} options - Additional options (contentType, etc.)
 */
const paginatedExtractionFlow = async (
  page,
  providedLink,
  existingLinks,
  progressCallback,
  ...
  const siteName = options.siteName || 'Unknown';

  try {
    while (pageCount < PAGE_TARGET) {
      progressCallback?.({
        progress: Math.round((pageCount / PAGE_TARGET) * 100),
        message: `Scraping ${siteName} page ${pageCount + 1}/${PAGE_TARGET}...`,
        isComplete: false,
        newItems: []
      });

      // Extract media from current page using provided extraction function
      const mediaObjects = await extractionFn(page, currentUrl);
      ...
      // Navigate to next page using provided next page function
      const nextPageUrl = nextPageFn(currentUrl, pageCount);
      ...
      pageCount++;
      console.log(`Navigated to next page: ${currentUrl}`);
    }
  } catch (error) {
    console.error(`${siteName} extraction error:`, error);
    progressCallback?.({
      progress: 100,
...
/**
 * Converts Safebooru thumbnail URL to real image URL
 * Pattern: /thumbnails/ -> //images/, remove "thumbnail_" prefix
 * Example:
 *   Input: https://safebooru.org/thumbnails/547/thumbnail_2d8ebf2865374e4a4ca64cbcbfe9fa2080c1de63.jpg?6279042
 *   Output: https://safebooru.org//images/547/2d8ebf2865374e4a4ca64cbcbfe9fa2080c1de63.jpg?6279042
 */
const convertSafebooruThumbnailToReal = (thumbnailUrl) => {
  try {
    // Replace /thumbnails/ with //images/
    let realUrl = thumbnailUrl.replace('/thumbnails/', '//images/');
    
    // Remove "thumbnail_" prefix from filename
    realUrl = realUrl.replace('/thumbnail_', '/');
    
    return realUrl;
  } catch (error) {
    console.error('Error converting Safebooru thumbnail URL:', error);
    return null;
  }
};

/**
 * Extracts image URLs from Safebooru feed page HTML
 * Searches for img src attributes, converts thumbnails to real image URLs
 * Detects the actual media type (jpg, png, gif, webm, mp4, etc.)
 * Extracts tags from the alt attribute (all go into general category)
 * Returns array of objects with correct image URLs and tags
 */
const extractMediaFromSafebooruFeed = async (page, link) => {
  const mediaObjects = [];

  try {
    // Get all img src and alt attributes from the page
    const imageData = await page.evaluate(() => {
      const imgs = Array.from(document.querySelectorAll('img'));
      return imgs
        .filter(img => img.src && img.src.includes('/thumbnails/') && img.src.includes('safebooru.org'))
        .map(img => ({
          src: img.src,
          alt: img.alt || '' // Extract alt text for tags
        }));
    });

    if (imageData.length === 0) {
      console.debug('No Safebooru thumbnail images found on feed page');
      return mediaObjects;
    }

    console.log(`✓ Found ${imageData.length} thumbnail image(s) on Safebooru feed page`);

    // Convert each thumbnail to base real image URL (without extension)
    const baseImageData = imageData
      .map(data => ({
        baseUrl: convertSafebooruThumbnailToReal(data.src),
        alt: data.alt
      }))
      .filter(data => data.baseUrl !== null);

    // Remove duplicates based on baseUrl
    const uniqueBaseUrls = new Map();
    baseImageData.forEach(data => {
      if (!uniqueBaseUrls.has(data.baseUrl)) {
        uniqueBaseUrls.set(data.baseUrl, data.alt);
      }
    });

    // Detect actual media type for each URL and create media objects
    for (const [baseUrl, altText] of uniqueBaseUrls) {
      try {
        // Parse tags from alt text
        const tags = altText
          .split(/[\s,]+/) // Split by spaces or commas
          .filter(tag => tag.length > 0 && tag.length < 100) // Filter out empty or very long strings
          .map(tag => tag.toLowerCase().trim());

        // Detect URL with tag-based optimization
        const detectedUrl = await detectSafebooruMediaType(page, baseUrl, tags);

        mediaObjects.push({
          postLink: link,
          videoLinks: [detectedUrl],
          tags: {
            author: [],
            copyright: [],
            character: [],
            general: tags // All tags go into general category
          }
        });
      } catch (error) {
        console.error(`Error processing ${baseUrl}:`, error.message);
        // Fallback: still add the media without tags if detection fails
        mediaObjects.push({
          postLink: link,
          videoLinks: [`${baseUrl}.jpg`],
          tags: {
            author: [],
            copyright: [],
            character: [],
    ...
    console.log(`✓ Converted and detected ${mediaObjects.length} image URL(s) with tags`);
  } catch (error) {
    console.error('Error extracting media from Safebooru feed:', error.message);
  }
...
/**
 * Detects the actual media type by checking tags first, then testing extensions
 * - If tags contain "animated_gif" -> .gif
 * - If tags contain "video" -> .mp4
 * - Otherwise, tests different extensions and checks for Safebooru's custom 404 page
 */
const detectSafebooruMediaType = async (page, baseUrl, tags = []) => {
  // Split URL and query string
  const [urlWithoutQuery, queryString] = baseUrl.includes('?') 
    ? baseUrl.split('?') 
    : [baseUrl, ''];
  ...
  // Check tags first for quick detection
  const tagsLower = tags.map(t => t.toLowerCase());
  
  if (tagsLower.includes('animated_gif')) {
    const url = `${urlWithoutExtension}.gif${queryString ? '?' + queryString : ''}`;
    console.log(`✓ Tag indicates GIF: ${url}`);
    return url;
  }
  
  if (tagsLower.some(t => t.includes('video'))) {
    const url = `${urlWithoutExtension}.mp4${queryString ? '?' + queryString : ''}`;
    console.log(`✓ Tag indicates MP4: ${url}`);
    return url;
  }
  
  // If no tag match, test different extensions using fast HEAD requests
  const mediaTypes = ['jpg', 'png'];
